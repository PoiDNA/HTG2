...
export default async function StaffClientsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  for (const b of (bookings || [])) {
    ...
    if (dateStr >= todayStr && b.status !== 'completed') { if (!c.nextSession || dateStr < c.nextSession) c.nextSession = dateStr; }
  }
            ...
            <div key={client.id} className="bg-htg-card border border-htg-card-border rounded-xl overflow-hidden">
              <div className="p-5 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4 min-w-0">
                  <div className="w-10 h-10 rounded-full bg-htg-sage/20 flex items-center justify-center shrink-0">
                    <span className="text-htg-sage font-bold text-sm">{client.name.charAt(0).toUpperCase()}</span>
                  </div>
                  <div className="min-w-0">
                    <p className="font-medium text-htg-fg truncate">{client.name}</p>
                    <p className="text-xs text-htg-fg-muted flex items-center gap-1"><Mail className="w-3 h-3" /> {client.email}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4 shrink-0 text-xs text-htg-fg-muted">
                  <span className="flex items-center gap-1"><Hash className="w-3 h-3" /> {client.totalSessions} sesji</span>
                  {client.nextSession && <span className="flex items-center gap-1 text-htg-sage font-medium"><Calendar className="w-3 h-3" /> Następna: {client.nextSession}</span>}
                </div>
              </div>
              <div className="border-t border-htg-card-border px-5 py-3 bg-htg-surface/30">
                ...
                {client.sessions[0]?.topics && <p className="text-xs text-htg-fg-muted mt-2 line-clamp-2">📝 {client.sessions[0].topics}</p>}
              </div>
            </div>
          ))}
        </div>

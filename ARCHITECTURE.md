...
| Tabela | Opis |
|--------|------|
| `staff_members` | Natalia, Agata, Justyna, Przemek (role, session_types) |
| `availability_rules` | Harmonogram tygodniowy (dzień + godziny) |
| `availability_exceptions` | Zablokowane dni |
| `booking_slots` | Konkretne sloty (data, czas, status, solo_locked; session_type obejmuje pre_session + natalia_interpreter) |
...
- `natalia_interpreter` — nowy session_type w booking_slots
- `interpreter_locale` kolumna na `bookings`
- Tłumaczki: Melania (EN), Bernadetta (DE), Edyta (PT)
- **Nie są w staff_members** — brak automatycznej weryfikacji dostępności (tech debt)
- Booking wymaga ręcznego potwierdzenia admina

...
export default async function MyActivationsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      <h2 className="text-xl font-serif font-semibold text-htg-fg mb-1">{t('activations_title')}</h2>
      <p className="text-sm text-htg-fg-muted mb-8">{t('activations_subtitle')}</p>
      ...
      {active.length === 0 ? (
        <div className="bg-htg-card border border-htg-card-border rounded-xl p-8 text-center mb-8">
          ...
          <p className="text-htg-fg-muted text-sm">{t('no_subscriptions')}</p>
        </div>
      ) : (
              ...
              <div key={ent.id} className="bg-htg-card border border-htg-card-border rounded-xl p-5 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-htg-fg-muted mt-0.5">
                    {t('valid_until', { date: formatDate(ent.valid_until, locale) })}
                  </p>
                </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-8">
        ...
        >
          ...
          <p className="font-semibold text-htg-fg group-hover:text-htg-indigo transition-colors">{t('session_with_natalia')}</p>
          <p className="text-xs text-htg-fg-muted mt-1">{t('buy_single_desc')}</p>
        ...
        {packages.find(p => p.slug === PRODUCT_SLUGS.MONTHLY) && (() => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs text-htg-fg-muted mt-1">{pkg.description || t('monthly_access')}</p>
              {pkg.prices?.[0] && (
                <p className="text-sm font-medium text-htg-indigo mt-3">
                  {formatPrice(pkg.prices[0].amount, pkg.prices[0].currency, locale)} / {t('per_month')}
                </p>
              )}
            </Link>
          );
        ...
        {packages.find(p => p.slug === PRODUCT_SLUGS.YEARLY) && (() => {
          ...
          return (
            ...
            >
              ...
              <p className="font-semibold text-htg-fg group-hover:text-htg-indigo transition-colors">{pkg.name}</p>
              <p className="text-xs text-htg-fg-muted mt-1">{pkg.description || t('yearly_access')}</p>
              {pkg.prices?.[0] && (
                <p className="text-sm font-medium text-htg-sage mt-3">
                  {formatPrice(pkg.prices[0].amount, pkg.prices[0].currency, locale)} / {t('per_year')}
                </p>
              )}
            </Link>
          );
      ...
      {expired.length > 0 && (
        <>
          <div className="border-t border-htg-card-border my-8" />
          <h3 className="text-sm font-semibold text-htg-fg-muted uppercase tracking-wider mb-3">{t('history')}</h3>
              ...
              return (
                <div key={ent.id} className="bg-htg-card border border-htg-card-border rounded-xl p-4 flex items-center gap-3 opacity-60">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-htg-fg-muted">{t('expired_at', { date: formatDate(ent.valid_until, locale) })}</p>
                  </div>

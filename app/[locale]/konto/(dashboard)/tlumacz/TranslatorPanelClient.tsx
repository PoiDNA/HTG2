...
interface TranslatorPanelClientProps {
  /**
   * Gdy admin podgląda panel tłumacza (impersonacja),
   * serwer przekazuje locale impersonowanego użytkownika.
   * Pomija wtedy sprawdzenie isTranslator po stronie klienta.
   */
  adminOverrideLocale?: string;
}

export default function TranslatorPanelClient({ adminOverrideLocale }: TranslatorPanelClientProps) {
  ...
  // Jeśli admin podgląda (adminOverrideLocale), używamy przekazanego locale.
  // W przeciwnym razie locale bierzemy z emaila zalogowanego użytkownika.
  const translatorLocale = adminOverrideLocale
    ?? (user?.email ? TRANSLATOR_LOCALE[user.email.toLowerCase()] : null);

  // Dostęp: albo prawdziwy tłumacz, albo admin z override
  const hasAccess = !!adminOverrideLocale || isTranslator;

  useEffect(() => {
    if (adminOverrideLocale) {
      // Admin preview — ładuj od razu, nie czekamy na useUserRole
      loadIssues();
      return;
    }
    if (roleLoading || !user) return;
    loadIssues();
  }, [roleLoading, user, adminOverrideLocale]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (adminOverrideLocale) return; // admin nie wysyła zgłoszeń w imieniu tłumacza
  ...
  if (!adminOverrideLocale && roleLoading) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-8">
      ...
      {/* Submit form — ukryty dla admina w trybie podglądu */}
      {!adminOverrideLocale && (

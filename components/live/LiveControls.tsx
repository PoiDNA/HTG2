...
export default function LiveControls({ backUrl = '/pl/konto/sesje-indywidualne' }: LiveControlsProps) {
  ...
  return (
    <div className="absolute top-4 left-4 right-4 z-30 flex items-center justify-between pointer-events-none">
      ...
      {/* Fullscreen toggle — always in top right */}
      <button
        onClick={toggleFullscreen}
        className={`pointer-events-auto flex items-center gap-2 px-4 py-2.5 rounded-xl backdrop-blur-md text-sm font-medium transition-all ${
          isFullscreen

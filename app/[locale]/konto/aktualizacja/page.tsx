...
import {
  ...
  ChevronDown, Trash2, BookOpen, Users, Heart,
} from 'lucide-react';
...
const STATUS_CONFIG = {
  pending: { label: 'Oczekujące', icon: Clock, color: 'text-yellow-500 bg-yellow-500/10 border-yellow-500/30' },
  approved: { label: 'Zaakceptowane', icon: CheckCircle, color: 'text-green-500 bg-green-500/10 border-green-500/30' },
  rejected: { label: 'Odrzucone', icon: XCircle, color: 'text-red-500 bg-red-500/10 border-red-500/30' },
};
...
export default function AccountUpdatePage() {
  ...
  async function loadData() {
    ...
    // Load existing requests, entitlements, and bookings in parallel
    ...
    <div className="space-y-8">
      ...
      <div className="bg-htg-card border border-htg-card-border rounded-2xl p-6">
        ...
        {entitlements.length === 0 && bookings.length === 0 ? (
          ...
          <div className="space-y-4">
            {/* Entitlements (sesje z biblioteki) */}
                  ...
                  {bookings.slice(0, 10).map(b => (
                    <div key={b.id} className="flex items-center justify-between py-2 px-3 bg-htg-surface rounded-lg">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        b.payment_status === 'confirmed_paid' ? 'bg-green-500/10 text-green-500' :
      ...
      {showForm && (
        <div className="bg-htg-card border-2 border-htg-sage/30 rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-htg-fg mb-4">Nowe zgłoszenie</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-htg-fg-muted mb-2">Opisz co kupiłeś/aś, np. konkretne sesje, miesiąc pakietu itp.</p>
              <textarea
                value={description}
            ...
            <div>
              <label className="block text-sm font-medium text-htg-fg mb-1">
                ...
                Potwierdzenie zakupu (opcjonalne)
              </label>
              <p className="text-xs text-htg-fg-muted mb-2">Załącz screenshot lub PDF potwierdzenia z WIX</p>
              <input
                ref={fileRef}
                type="file"
                accept="image/*,.pdf"
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-htg-surface border border-dashed border-htg-card-border rounded-xl text-sm text-htg-fg-muted hover:border-htg-sage/50 transition-colors"
              >
      ...
      {requests.length > 0 && (
        <div className="bg-htg-card border border-htg-card-border rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-htg-fg mb-4">Twoje zgłoszenia ({requests.length})</h3>
          <div className="space-y-3">
            {requests.map(r => {
              const sc = STATUS_CONFIG[r.status];
              const Icon = sc.icon;
              return (
                <div key={r.id} className={`border rounded-xl p-4 ${sc.color}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs opacity-70">{formatCategoryLabel(r.category)}</span>
                      </div>
                      <p className="text-sm text-htg-fg">{r.description}</p>
                      {r.purchase_date && (
                        <p className="text-xs text-htg-fg-muted mt-1">Data zakupu: {new Date(r.purchase_date).toLocaleDateString('pl')}</p>
                      )}
                      {r.proof_filename && (
                        <p className="text-xs text-htg-fg-muted mt-1 flex items-center gap-1">
                          <FileText className="w-3 h-3" />
                          Załącznik: {r.proof_filename}
                        </p>
                      )}
                      {r.admin_notes && (
                        <div className="mt-2 p-2 bg-htg-surface/50 rounded-lg">
                          <p className="text-xs font-semibold text-htg-fg-muted">Odpowiedź admina:</p>

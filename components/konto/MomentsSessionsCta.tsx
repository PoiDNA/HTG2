...
function Gear({ id, palette, phase }: { id: string; palette: GearPalette; phase: number }) {
...
function GearBadge({ palette, phase, gearId, chevron }: { palette: GearPalette; phase: number; gearId: string; chevron: 'left' | 'right' }) {
  const Chevron = chevron === 'right' ? ChevronRight : ChevronLeft;
  return (
    <div
      className="relative shrink-0 w-[72px] h-[72px] sm:w-24 sm:h-24 transition-[filter] duration-200 group-hover:brightness-[1.06] group-focus-visible:brightness-[1.06]"
      style={{ filter: `drop-shadow(0 10px 18px ${palette.dark}55)` }}
    >
      <Gear id={gearId} palette={palette} phase={phase} />
      <Chevron
        className="absolute inset-0 m-auto w-7 h-7 sm:w-8 sm:h-8 text-white pointer-events-none"
        strokeWidth={2.6}
...
const ctaClasses = 'group flex items-center gap-3 sm:gap-4 select-none transition-transform duration-150 active:scale-[0.97]';
  ...
  return (
    <div className="flex items-center justify-center">
      <Link href="/konto/sluchaj" aria-label="Sesje" className={ctaClasses}>
        <Label text="Sesje" />
        <GearBadge palette={SAGE} phase={0} gearId="gear-sessions" chevron="right" />
      </Link>
      <div className="-ml-3 sm:-ml-4">
        <Link href="/konto/momenty" aria-label="Momenty" className={ctaClasses}>
          <GearBadge palette={VIOLET} phase={15} gearId="gear-moments" chevron="left" />
          <Label text="Momenty" />
        </Link>
      </div>
    </div>
